...
function UserGroupUpdate(props: any) {
  ...
  React.useEffect(() => {
    if (selectGroupID) {
      setGroupId(selectGroupID.groupId)
      setGroupname(selectGroupID.groupName)
      setDescription(selectGroupID.groupDesc)
      setLocationNames(
        selectGroupID.locationHierarchy.map((location: any) => {
          return {
            label: location.hierarchyId,
            value: location.hierarchyId,
      ...
      if (selectGroupID.productHierarchy.length > 0) {
        switch (selectGroupID.productHierarchy[0].hierarchyLevel) {
          case 'division':
            setDisabled(false)
            // setPayload('')
            // setDivpay(payload)
...
const mapStatetoProps = (state: any) => {
  return {
    groupDetails: state.manageGroupReducer.groupDetails,
...
const matchDispatchToProps = (dispatch: any) => {
  return {
    reset_groupID: () => dispatch(reset_groupID()),
  }
}
export default connect(mapStatetoProps, matchDispatchToProps)(UserGroupUpdate)
